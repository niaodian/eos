...
版本化：`docs/eos/VERSION`（当前 `eos-1.5.0`）。升级用 `degit` 拉新版到 /tmp 后 `diff -ru` 合并，
...
| 7 Testing | 按测试策略验证+spec↔test 可追溯+**NFR 验证**+**规范对齐量化** | `bmad-tea`(Murat)、`bmad-testarch-trace`、`bmad-testarch-nfr`、`bmad-qa-generate-e2e-tests`、`/spec-align` | G7：每条验收≥1测试且全绿；**NFR 已验**；**spec-alignment：AC 覆盖率/一次过率/无漂移** | `I:testing` | trace 矩阵确保无未测 AC；**spec-align 量化 Agent 输出的规范对齐度与一次过率** |
...
| — | 合规代码起步骨架（🟡→脚手架） | skill `.github/skills/eos-compliance-skeletons/` + `docs/eos/examples/compliance-starter/`（redaction/consent/DSAR/audit，**Node/ESM + Python/stdlib 双实现**，零依赖可跑；redaction 即 D5 数据出境门的代码形态，按 HIPAA/PCI/GDPR-PIPL 分档） | 新建补强 |
