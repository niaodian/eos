...
npx degit niaodian/eos#eos-1.18.0 my-app
...
Versioning: `docs/eos/VERSION` (current `eos-1.18.0`). For upgrades, use `degit` to pull the new version to /tmp, merge with `diff -ru`,
...
| D27 | The stage agents were told to run commands they had no permission to run (eos-1.16.0) | A developer running a real project reported that every stage ended the same way: no preview of what was written, no confirmation, no gate run, no statement of what came next — so they had to ask, each time, where they were. Most of it reduced to one configuration defect. Four stage agents and seven prompts carried a "Return protocol (do not skip)" instructing them to run `eos check` and `eos next`, while their front matter granted only `search` + `editFiles`: they were **required to execute a command they had no permission to execute**, and the honest fallback ("this session has no terminal tool") looked to the user like a missing feature. `runCommands` was granted, handoff prompts became real slash commands rather than prose about them, and a new always-on rule (`05-stage-closeout`) binds every stage — the default agent included, which is why the earlier per-agent version of this rule never took effect — to **preview → confirm → gate → announce**, plus a real conversation before any one-way door. Three further defects were fixed against the code rather than the report. (1) G4 verified that the tech-stack ADR existed but never that the decision reached `00-workspace.instructions.md`; since no later agent reads an ADR and all of them read the always-on rule, a surviving ⛳ PROVISIONAL marker kept telling them to run `npm ci` on a Python project — now `stack-landed-in-workspace-rule` (NOT_APPLICABLE when the rule is absent, because the failure being prevented is a *contradiction*, not a missing file). (2) `story-ready`/`verified` bound the **whole** `docs/prd.md`, so adding one criterion made all 20 stories' evidence stale and specifying a backlog up front cost a re-run per story; both gates only ever ask about the story's own ids, so they now bind the digest of the criteria each story **cites**, via the `collections` mechanism that already solved exactly this for `release-ready` — no schema migration. A journey test failed on this change because it had encoded the old behaviour, which was the clearest evidence the fix was right. (3) `/eos-init` verified branch protection with the legacy endpoint, which 404s for a ruleset and reported protected repositories as unprotected; it now uses `rules/branches/main`, leads with `Enforcement: Active` (a ruleset defaults to *Disabled* and enforces nothing), states the GitHub Free private-repo limits instead of walking a click-path that cannot work, creates the remote, and re-runs the ledger check itself. Also: an optional BCP-47 `language` in `.eos/project.json`, validated rather than free text. 292 tests | Developer-experience patch (first real-project feedback) |
