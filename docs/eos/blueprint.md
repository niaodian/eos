...
版本化：`docs/eos/VERSION`（当前 `eos-1.4.5`）。升级用 `degit` 拉新版到 /tmp 后 `diff -ru` 合并，
...
| D14 | 受监管行业合规清单 + 制度前置 + Agentic 数据出境门 | `docs/checklists/F-compliance.md`；`/compliance` + `/requirements` Step 2.5；`eos-doctor` D5 | 新建 |
