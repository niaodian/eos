...
模式就是整个项目的门禁策略，由 `.eos/project.json` 中的 `workflowProfile` 选择，并以数据形式定义在
`.eos/workflow.json` 里。所有模式跑的是同一个引擎，区别只在策略，因此升级一档是改一行，而不是一次迁移。

| 模式 | `workflowProfile` | 适用于 | 相比下一档增加了什么 |
|---|---|---|---|
| Lite | `prototype` | 个人项目、PoC、小型工具 | 除本地激活外不设任何门禁；一切变更都是 Spike，且不可合并 |
| Standard | `standard-product` | 一般团队产品 | 完整 SDLC：从发现到迭代、逐 Story 验证、发布验证，保留一个可 Waive 的出口 |
| Controlled | `controlled` | 企业级与业务关键系统 | 任何门禁都不可被 Waive；热修复同样必须证明它有效、且交付物与被验证的一致；发布必须规划遥测 |
| Regulated | `regulated` | 合规与审计敏感系统 | 每次变更都要记录如此分类的理由；Spike 不可合并；必须完成迭代回写；连纯文档变更也不能关掉激活门禁 |

逐档、逐 Change Type、逐门禁地看，每一档都**严格不弱于**下一档，并且有测试断言这一性质 —— 一架上层
反而比下层更宽松的成熟度阶梯，比没有阶梯更糟，因为采用它会在无声中放松一项控制。

升级是渐进的，而且只在这个方向上安全。改掉那一个字段，然后重新验证：

```bash
node .github/eos/eos.mjs verify --plan   # 更严策略下现在会要求什么
node .github/eos/eos.mjs verify --full   # 在新档位下重跑全部
```

旧档位记录为 `NOT_APPLICABLE` 的门禁会变为 `PENDING`，而不是凭一个在更低标准下做出的决定直接通过。
