...
> 版本：与 `docs/eos/VERSION` 同步（当前 `eos-1.17.0`）
...
|---|---|
| **目标** | 从模板得到一个配置健康的空项目 |
| **怎么启动** | `npx degit niaodian/eos#eos-1.17.0 my-app && cd my-app` |
...
1. 所有人从**同一个 release tag**（`eos-1.17.0`）开始。默认分支会持续变动，
...
```
# ── 终端 —— 唯一循环（日常只需要这些）──
npx degit niaodian/eos#eos-1.17.0 my-app   # 新建项目
